...
<div class="kt-row">
	<div class="column kt-col-xs-12">
		<?php kando_show_alerts('subscriptions'); ?>
	</div>
</div>
...
<div class="page-options d-flex tabs-wrapper">
	<ul class="list-inline mb-0 order_btn_group nav">
		<li class="list-inline-item nav-select" style="margin-left: -5px;">
			<a class="nav-link <?= (!isset($_GET['section'])) ? 'active' : '' ?>" href="<?= home_url('dashboard/?action=subscriptions') ?>">
				<i class="far fa-list-ul" style="margin-right:5px;"></i><?php _e('All', SAMYAR_TEXT_DOMAIN);
				echo '<span class="button button-light badge-error-orders">' . get_count_dripfeed_orders('all') . '</span>' ?></a></li>
		<?php
		$status_array =array(
			'active',
			'paused',
			...
			foreach ($status_array as $row_status) {
				...
				<li class="list-inline-item nav-select">
					<a class="nav-link <?= (isset($_GET['status']) && $_GET['status'] === $row_status) ? 'active' : '' ?>"
					   href="<?= home_url('dashboard/?action=subscriptions&status=' . $row_status) ?>"><i class="<?= $icon ?>"
						...
						echo '<span class="button ' . $color . ' badge-error-orders">' . get_count_subscriptions_orders($row_status) . '</span>';
						//						}
						?>
					</a>
				</li>
...
<div class="dashboard-posts-box dashboard-tickets-box">
	<div class="dashboard-posts-title-holder">
		...
		<h5 class="dashboard-posts-title"><?php _e("Subscriptions order", SAMYAR_TEXT_DOMAIN); ?></h5>
	</div>
	<div class="dashboard-posts-list" style="padding-top: 10px;">
		...
		$query['service_type']='subscriptions';
		...
		<?php
			...
			?>
			<span class="orders-notfound"><?php _e("No subscription order has been added yet.", SAMYAR_TEXT_DOMAIN); ?></span>
		<?php
		endif;
		?>
	</div>
