                ...
                <div class="clearfix">
                    ...
                    <div class="column kt-col-xs-6 kt-col-md-3 float-right">
                        ...
                        <label style="margin: 20px 0;font-size: 15px;font-weight: bold;" class="publish-notification"
                               for="success-status">فقط موفق ها</label>
                    </div>
                    <!--
                    <div class="column kt-col-xs-6 kt-col-md-3 float-right">
    ...
    <ul class="list-inline mb-0 order_btn_group nav">
        <li class="list-inline-item nav-select" style="margin-left: -5px;">
            <a class="nav-link <?= (!isset($_GET['section'])) ? 'active' : '' ?>"
               href="<?= home_url('dashboard/?action=payments') ?>">
                <?php _e('All', SAMYAR_TEXT_DOMAIN);
                echo '<span class="button button-light badge-error-orders">' . get_count_payments('all') . '</span>' ?></a>
        </li>
        <?php
...
<div class="dashboard-posts-box dashboard-tickets-box">
    <div class="dashboard-posts-title-holder">
        ...
        <h5 class="dashboard-posts-title"><?php _e("Credit payments", SAMYAR_TEXT_DOMAIN); ?></h5>
    </div>
    <div class="dashboard-posts-list">
        ...
        $query['payment_type'] = [
            'operator' => 'IN',
            'value' => ['add-credit', 'set-credit', 'deleted', 'bonus','coupon'],
        ];
        //        $query['order_id'] = NULL;
                ...
                foreach ($payments as $payment) {
                    ...
                    <tr id="order-<?php echo esc_attr($payment->id) ?>">
                        <td data-title="<?php _e("Type", SAMYAR_TEXT_DOMAIN); ?>"
                            class="<?php if ($payment->status == 1): ?>success-payment<?php else: ?>fail-payment<?php endif; ?>">
                            <?php //if ($payment->status == 1): ?>
                            <?php if ($payment->payment_type == "add-credit"): ?>
                                <span style="color: #00a699;font-size: 20px;"
                                      data-tooltip="<?php _e("Add funds", SAMYAR_TEXT_DOMAIN); ?>"><i
                                            class="fal fa-plus"></i></span>
                            <?php elseif ($payment->payment_type == "decrease-credit"): ?>
                                <span style="color: #e60921;font-size: 20px;"
                                      data-tooltip="<?php _e("Decrease credit", SAMYAR_TEXT_DOMAIN); ?>"><i
                            ...
                            <?php elseif ($payment->payment_type == "order"): ?>
                                <span style="color: #e60921;font-size: 20px;"><i class="fal fa-minus"
                                                                                 data-tooltip="<?php _e("Credit reduction for order", SAMYAR_TEXT_DOMAIN); ?>"></i></span>
                            <?php elseif ($payment->payment_type == "refund"): ?>
                                <span style="color: #00a699;font-size: 23px"
                                      data-tooltip="<?php _e("Refund", SAMYAR_TEXT_DOMAIN); ?>"><i
                                            class="fal fa-undo"></i></span>
                            <?php elseif ($payment->payment_type == "bonus"): ?>
                                <span style="color: #00a699;font-size: 23px"
                                      data-tooltip="<?php _e("Bonus", SAMYAR_TEXT_DOMAIN); ?>"><i
                                            class="fal fa-award"></i></span>
                            <?php elseif ($payment->payment_type == "coupon"): ?>
                                <span style="color: #00a699;font-size: 23px"
                                      data-tooltip="<?php _e("Coupon", SAMYAR_TEXT_DOMAIN); ?>"><i
                                            class="fal fa-gift"></i></span>
                            <?php endif; ?>
                        </td>
                        ...
                        <td data-title="<?php _e("Description", SAMYAR_TEXT_DOMAIN); ?>">
                            ...
                            if ($coupon_code) {
                                ...
                                <span class="button button-green badge-error-orders"
                                      style="margin-right: 0;"> <?php _e("Amount charged:", SAMYAR_TEXT_DOMAIN); ?> <?= priceController::kandoFormatPrice($original_price_val)['price_for_show_formatted'] ?></span>
                                <br>
                            ...
                            if ($credit_before_charge) {
                                ?>
                                <?php _e("Credit before charging:", SAMYAR_TEXT_DOMAIN); ?> <?= priceController::kandoFormatPrice($credit_before_charge->meta_value)['price_for_show_formatted'] ?>
                                <br>
                            <?php }
                            ?>
                        ...
                        <?php if (kando_user_can('edit_payment')): ?>
                            <td data-title="<?php _e("Operations", SAMYAR_TEXT_DOMAIN); ?>">
                                ...
                                <a href="#" class="button button-green kando-change-payment-status"
                                   data-id="<?php echo $payment->id ?>"
                                   data-status="<?php echo $payment->status ?>"><?php _e("Edit", SAMYAR_TEXT_DOMAIN); ?></a>
