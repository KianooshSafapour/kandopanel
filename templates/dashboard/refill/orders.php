...
$title =  __('your Refill orders', SAMYAR_TEXT_DOMAIN);
?>
<div class="kt-row">
    <div class="column kt-col-xs-12">
        <?php kando_show_alerts('refill'); ?>
    </div>
</div>

<div class="tickets-navigation">
    <a href="#" class="button button-blue kando-show-refill-order-filter" data-wpel-link="internal"><?php _e("Filter", SAMYAR_TEXT_DOMAIN); ?></a>
</div>

<div class="kt-row">
    <div class="kt-col-xs-12 kt-col-md-12 float-right">
        <form method="POST" class="samyar-form filter-refill-orders-form" style="display: none">
            <input type="hidden" name="action" value="samyar_filter_refill_orders_form">
...
<div class="page-options d-flex tabs-wrapper">
    <ul class="list-inline mb-0 order_btn_group nav">
        <li class="list-inline-item nav-select" style="margin-left: -5px;">
            <a class="nav-link <?= (!isset($_GET['section'])) ? 'active' : '' ?>" href="<?= home_url('dashboard/?action=refill') ?>">
                <i class="far fa-list-ul" style="margin-right:5px;"></i><?php _e('All', SAMYAR_TEXT_DOMAIN);
                echo '<span class="button button-light badge-error-orders">' . get_count_refill_orders('all') . '</span>' ?></a></li>
        <?php
        ...
        if (!empty($status_array)) {
            foreach ($status_array as $row_status) {
                ...
                <li class="list-inline-item nav-select">
                    <a class="nav-link <?= (isset($_GET['status']) && $_GET['status'] === $row_status) ? 'active' : '' ?>" href="<?= home_url('dashboard/?action=refill&status=' . $row_status) ?>">
                        <i class="<?= $icon ?>" style="margin-right:5px;margin-top: 3px;"></i>
                        <?= samyar_order_status_title($row_status) ?>
                        <?php
                        echo '<span class="button ' . $color . ' badge-error-orders">' . get_count_refill_orders($row_status) . '</span>';
                        ?>
                    </a>
                </li>
            <?php }
...
<div class="dashboard-posts-box dashboard-tickets-box">
    <div class="dashboard-posts-title-holder">
        ...
        <?php if (isset($_GET['status']) && !empty($_GET['status']) && $_GET['status'] === "error"): ?>
            <a href="#" style="float: left;margin-right: 5px" class="button button-red kt-ajax-button samyar-resend-refill-orders" data-wpel-link="internal"><?php _e("Resend all errors", SAMYAR_TEXT_DOMAIN); ?></a>
        <?php endif; ?>
    </div>
    <div class="dashboard-posts-list" style="padding-top: 10px;">
        ...
        $offset = ($limit * $paged) - $limit;
            ...
            $refill_orders = Refill::where($query);
        ...
        if ($refill_orders):
            ...
            <table class="shop_table shop_table_responsive">
                ...
                <tbody>
                <?php
                foreach ($refill_orders as $refill_order):
                    include('order.php');
                endforeach; ?>
                </tbody>
            </table>
